...
const promoPackagingFee=order=>Number(order?.promo_packaging_fee||0)
const promoPackagingCount=order=>Number(order?.promo_packaging_count||0)
...
function printSaleTicket(order){
  ...
  if(promoPackagingFee(order)>0)lines.push(...thermalPair(`EMPAQUE 3x2 (${promoPackagingCount(order)} x $14)`,thermalMoney(promoPackagingFee(order))))
  if(Number(order.delivery_fee||0)>0)lines.push(...thermalPair(order.delivery_zone_name?`ENVIO ${order.delivery_zone_name.toUpperCase()}`:'ENVIO',thermalMoney(order.delivery_fee)))
...
function KitchenOrderDetail({order,onClose,onPrintKitchen,onPrintSale,onStatusAction}){
  ...
  return <div className="kitchen-detail-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className="kitchen-detail-modal">
    ...
    <div className="kitchen-detail-totals"><div><span>Productos antes de promo</span><strong>{money(subtotalBeforePromo(order))}</strong></div>{promoDiscount(order)>0&&<div className="promo-free-detail"><span>Promo 3×2 · producto(s) base gratis</span><strong>-{money(promoDiscount(order))}</strong></div>}<div><span>Subtotal después de promo</span><strong>{money(order.subtotal)}</strong></div>{promoPackagingFee(order)>0&&<div><span>Empaque promo 3×2 · {promoPackagingCount(order)} × $14</span><strong>{money(promoPackagingFee(order))}</strong></div>}{Number(order.delivery_fee||0)>0&&<div><span>Envío{order.delivery_zone_name?` · ${order.delivery_zone_name}`:''}</span><strong>{money(order.delivery_fee)}</strong></div>}<div><span>Venta KYO</span><strong>{money(order.total)}</strong></div>{Number(order.tip_amount||0)>0&&<div><span>Propina</span><strong>{money(order.tip_amount)}</strong></div>}<div className="grand"><span>{paid?'TOTAL COBRADO':'TOTAL A COBRAR'}</span><strong>{money(chargedTotal(order))}</strong></div></div>
