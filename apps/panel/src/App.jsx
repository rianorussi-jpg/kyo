...
function useCatalog(){
  ...
  const [settings,setSettings]=useState({minimum_order:200,points_reward_cost:250,points_reward_product_id:null,business_hours:null,delivery_riders:defaultDeliveryRiders,promo_3x2_enabled:false,promo_3x2_days:['tue','wed','thu']})
  const refresh=async()=>{
    if(!supabase)return
    ...
    if(s)setSettings({minimum_order:Number(s.minimum_order||200),points_reward_cost:Number(s.points_reward_cost||250),points_reward_product_id:s.points_reward_product_id||null,business_hours:s.business_hours||defaultBusinessHours,delivery_riders:s.delivery_riders||defaultDeliveryRiders,promo_3x2_enabled:!!s.promo_3x2_enabled,promo_3x2_days:s.promo_3x2_days||['tue','wed','thu']})
  }
  useEffect(()=>{refresh()},[])
