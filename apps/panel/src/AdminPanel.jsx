  ...
  return <div className="record-detail-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <section className="record-detail-modal">
      ...
      <div className="record-detail-grid">
        ...
        <article><small>TIPO</small><strong>{order.fulfillment_type==='delivery'?'Delivery':'Pickup'}</strong><span>{order.payment_method==='card'?'Tarjeta / Stripe':'Efectivo'}</span></article>
        <article><small>ESTADO</small><strong>{statusLabels[order.status]||order.status}</strong><span>{order.payment_status==='paid'?'Pago confirmado':order.payment_status==='failed'?'Pago fallido':order.payment_method==='cash'?'Pago en efectivo':order.payment_status||''}</span></article>
      </div>
