...
function AdminRecords({orders,fixedBranch=null}){
  ...
  return <div className="admin-records">
    ...
    <div className="records-table tip-records-table"><div className="records-head"><span>Pedido</span><span>Fecha</span><span>Sucursal</span><span>Cliente</span><span>Estado</span><span>Venta</span><span>Propina</span><span></span></div>{filtered.map(o=><div className="records-row" key={o.id}><strong>#{String(o.order_number).padStart(4,'0')}</strong><span>{new Date(o.created_at).toLocaleString('es-MX')}</span><span><b className={`branch-pill small ${o.branch_id}`}>{o.branch_id==='zakia'?'Zákia':'Milenio'}</b></span><span>{o.profiles?.full_name||'Cliente KYO'}</span><span>{statusLabels[o.status]}</span><strong>{money(o.total)}</strong><strong className={Number(o.tip_amount||0)>0?'record-tip':'record-no-tip'}>{Number(o.tip_amount||0)>0?money(o.tip_amount):'—'}</strong><button className="record-detail-btn" onClick={()=>setDetailOrder(o)}>Ver detalles</button></div>)}{!filtered.length&&<div className="records-empty">No hay registros para este periodo.</div>}</div>
...
function RecordDetailModal({order,onClose}){
  ...
  return <div className="record-detail-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <section className="record-detail-modal">
      ...
      <div className="record-detail-totals"><div><span>Venta KYO <small>(sin propina)</small></span><strong>{money(sale)}</strong></div><div><span>Propina</span><strong>{tip>0?money(tip):'—'}</strong></div><div className="grand"><span>{order.payment_method==='card'?'Total cobrado':'Total del pedido'}</span><strong>{money(charged)}</strong></div></div>
    </section>
  </div>
}
