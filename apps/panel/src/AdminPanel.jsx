  ...
  return <>
    ...
    <div className="admin-orders">
      {filtered.length===0?<div className="admin-empty"><Package/><h3>No hay pedidos</h3><p>No hay pedidos para este filtro.</p></div>:filtered.map(o=><article key={o.id}>
        <div className="admin-order-top"><span><small>PEDIDO</small><strong>#{String(o.order_number).padStart(4,'0')}</strong></span><span className={`branch-pill ${o.branch_id}`}>{o.branch_id==='zakia'?'ZÁKIA':'MILENIO'}</span><span className={`admin-status ${o.status}`}>{statusLabels[o.status]}</span><span className="admin-client-detail"><small>CLIENTE</small><strong>{o.profiles?.full_name||'Cliente KYO'}</strong>{o.profiles?.phone&&<em>{o.profiles.phone}</em>}</span><span><small>TOTAL SIN PROPINA</small><strong>{money(o.total)}</strong><em className="admin-shipping-amount">Productos {money(o.subtotal)}{Number(o.delivery_fee||0)>0?` · Envío ${money(o.delivery_fee)}`:' · Pickup sin envío'}</em>{Number(o.tip_amount||0)>0&&<em className="admin-tip-amount">+ {money(o.tip_amount)} propina · Cobro {money(Number(o.total||0)+Number(o.tip_amount||0))}</em>}</span></div>
...
function AdminRecords({orders,fixedBranch=null}){
  const [branch,setBranch]=useState(fixedBranch||'all'); const [period,setPeriod]=useState('1'); const [detailOrder,setDetailOrder]=useState(null)
  useEffect(()=>{if(fixedBranch)setBranch(fixedBranch)},[fixedBranch])
  ...
  return <div className="admin-records">
    <div className="records-toolbar"><div><small>SUCURSAL</small>{fixedBranch?<div className={`fixed-branch-label ${fixedBranch}`}><MapPin/> KYO {branchName}</div>:<BranchFilter value={branch} onChange={setBranch}/>}</div><div><small>PERIODO</small><div className="admin-filter-row">{[['1','1 día'],['7','7 días'],['30','30 días'],['all','Toda la vida']].map(([v,l])=><button key={v} className={period===v?'active':''} onClick={()=>setPeriod(v)}>{l}</button>)}</div></div></div>
    <div className="record-summary extended tips"><article><DollarSign/><span><small>VENTAS KYO (SIN PROPINAS)</small><strong>{money(total)}</strong></span></article><article className="tip-kpi"><Gift/><span><small>PROPINAS</small><strong>{money(tips)}</strong></span></article><article><Package/><span><small>PEDIDOS</small><strong>{filtered.length}</strong></span></article><article><Truck/><span><small>DELIVERY</small><strong>{delivery}</strong></span></article><article><Store/><span><small>PICKUP</small><strong>{pickup}</strong></span></article><article><ClipboardList/><span><small>ENTREGADOS</small><strong>{delivered}</strong></span></article></div>
    <div className="records-table tip-records-table"><div className="records-head"><span>Pedido</span><span>Fecha</span><span>Sucursal</span><span>Cliente</span><span>Estado</span><span>Total</span><span>Propina</span><span></span></div>{filtered.map(o=><div className="records-row" key={o.id}><strong>#{String(o.order_number).padStart(4,'0')}</strong><span>{new Date(o.created_at).toLocaleString('es-MX')}</span><span><b className={`branch-pill small ${o.branch_id}`}>{o.branch_id==='zakia'?'Zákia':'Milenio'}</b></span><span>{o.profiles?.full_name||'Cliente KYO'}</span><span>{statusLabels[o.status]}</span><strong>{money(o.total)}</strong><strong className={Number(o.tip_amount||0)>0?'record-tip':'record-no-tip'}>{Number(o.tip_amount||0)>0?money(o.tip_amount):'—'}</strong><button className="record-detail-btn" onClick={()=>setDetailOrder(o)}>Ver detalles</button></div>)}{!filtered.length&&<div className="records-empty">No hay registros para este periodo.</div>}</div>
    {detailOrder&&<RecordDetailModal order={detailOrder} onClose={()=>setDetailOrder(null)}/>}  
  </div>
}

function RecordDetailModal({order,onClose}){
  const tip=Number(order.tip_amount||0)
  const sale=Number(order.total||0)
  const charged=sale+tip
  return <div className="record-detail-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <section className="record-detail-modal">
      <div className="record-detail-head"><div><small>DETALLE DEL PEDIDO</small><h2>Pedido #{String(order.order_number).padStart(4,'0')}</h2><p>{new Date(order.created_at).toLocaleString('es-MX')} · KYO {order.branch_id==='zakia'?'Zákia':'Milenio'}</p></div><button onClick={onClose} aria-label="Cerrar"><X/></button></div>
      <div className="record-detail-grid">
        <article><small>CLIENTE</small><strong>{order.profiles?.full_name||'Cliente KYO'}</strong>{order.profiles?.phone&&<span>{order.profiles.phone}</span>}</article>
        <article><small>TIPO</small><strong>{order.fulfillment_type==='delivery'?'Delivery':'Pickup'}</strong><span>{order.payment_method==='card'?'Tarjeta / Stripe':order.payment_method==='terminal'?'Terminal':'Efectivo'}</span></article>
        <article><small>ESTADO</small><strong>{statusLabels[order.status]||order.status}</strong><span>{order.payment_status==='paid'?'Pago confirmado':order.payment_status==='failed'?'Pago fallido':order.payment_method==='cash'?'Pago en efectivo':order.payment_method==='terminal'?'Pago con terminal al recibir':order.payment_status||''}</span></article>
      </div>
      {(order.delivery_address||order.delivery_reference||order.delivery_notes)&&<div className="record-detail-address"><small>ENTREGA / NOTAS</small>{order.delivery_address&&<strong>{order.delivery_address}</strong>}{order.delivery_reference&&<span><b>Referencia:</b> {order.delivery_reference}</span>}{order.delivery_notes&&<span><b>Notas:</b> {order.delivery_notes}</span>}</div>}
      <div className="record-detail-items"><div className="record-detail-section-title"><small>PRODUCTOS</small><strong>{order.order_items?.reduce((a,i)=>a+Number(i.quantity||0),0)||0} unidades</strong></div>{order.order_items?.map(i=><article key={i.id}><div><strong>{i.quantity}× {i.product_name}</strong>{i.item_note&&<em>Nota: {i.item_note}</em>}</div>{i.customizations?.length>0&&<div className="record-detail-customs">{Object.entries(i.customizations.reduce((g,c)=>{const title=c.template_name||c.group_name||c.customization_name||c.title||'Personalización';(g[title]??=[]).push(c);return g},{})).map(([title,rows])=><span key={title}><b>{title}:</b> {rows.map(c=>c.label||c.option_name||c.name).join(', ')}</span>)}</div>}</article>)}</div>
      <div className="record-detail-totals"><div><span>Productos</span><strong>{money(Number(order.subtotal||0)+Number(order.promo_discount||0))}</strong></div>{Number(order.promo_discount||0)>0&&<div><span>Promo 3×2</span><strong>-{money(order.promo_discount)}</strong></div>}<div><span>Subtotal productos</span><strong>{money(order.subtotal)}</strong></div>{Number(order.promo_packaging_fee||0)>0&&<div><span>Empaque promo 3×2 · {Number(order.promo_packaging_count||0)} × $14</span><strong>{money(order.promo_packaging_fee)}</strong></div>}<div><span>Envío{order.delivery_zone_name?` · ${order.delivery_zone_name}`:''}</span><strong>{Number(order.delivery_fee||0)>0?money(order.delivery_fee):'—'}</strong></div><div><span>Total sin propina</span><strong>{money(sale)}</strong></div><div><span>Propina</span><strong>{tip>0?money(tip):'—'}</strong></div><div className="grand"><span>{order.payment_method==='card'?'Total cobrado':'Total del pedido'}</span><strong>{money(charged)}</strong></div></div>
    </section>
  </div>
}
...
function AdminStats({orders,fixedBranch=null}){
  ...
  const exportRecords=()=>{
    const rows=[['Pedido','Fecha','Sucursal','Tipo','Estado','Cliente','Teléfono','Subtotal','Envío','Venta KYO','Propina %','Propina','Total cobrado','Productos','Dirección']]
...
function AdminMenuManager({catalog,onEdit,fixedBranch=null}){
  ...
  const setKitchenTemp=async(product,kitchenTemp)=>{
    const key=`temp:${product.id}`;setSaving(key)
    const {error}=await supabase.from('products').update({kitchen_temp:kitchenTemp}).eq('id',product.id)
    setSaving('')
    if(error)return alert(error.message)
    await catalog.refresh()
  }
      ...
      ?<div className="admin-menu-by-category">
        {groups.map(({cat,products,subs})=><section className="admin-menu-category" key={cat.id}>
          ...
          {subs.length>0
            ...
            :<div className="admin-product-grid">{products.map(product=><AdminProductCard key={product.id} product={product} branches={branches} saving={saving} setProductBranch={setProductBranch} setPromoProduct={!fixedBranch?setPromoProduct:null} setKitchenTemp={!fixedBranch?setKitchenTemp:null} onEdit={onEdit}/>)}</div>}
        </section>)}
      </div>
...
function AdminProductCard({product,branches,saving,setProductBranch,setPromoProduct,setKitchenTemp,onEdit}){
  return <article className={`admin-product-card-branch ${!product.available?'disabled':''}`}>
    ...
    <div className="admin-product-card-info">
      ...
      {setKitchenTemp&&<div className="kitchen-temp-admin"><span><small>ESTACIÓN DE COCINA</small><strong>{product.kitchen_temp==='F'?'Frío':'Caliente'}</strong></span><div><button type="button" className={product.kitchen_temp==='F'?'active cold':''} disabled={saving===`temp:${product.id}`} onClick={()=>setKitchenTemp(product,'F')}>F</button><button type="button" className={product.kitchen_temp!=='F'?'active hot':''} disabled={saving===`temp:${product.id}`} onClick={()=>setKitchenTemp(product,'C')}>C</button></div></div>}
