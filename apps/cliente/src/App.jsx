...
function friendlyError(error,context='general'){
  ...
  const safeBackendMessages=[
    ...
    'colonia',
    'dirección no tiene una colonia'
  ]
...
function usePromo3x2(cart){
  ...
  const eligibleUnits=status.active?(cart||[]).reduce((sum,i)=>sum+(!i.reward&&i.promo_3x2_eligible?Math.max(1,Number(i.qty||1)):0),0):0
  const packagingCount=Math.floor(eligibleUnits/3)
  const packagingFee=packagingCount*14
  return {...status,discount,packagingCount,packagingFee,refresh:refreshStatus}
}
...
function App(){
  ...
  const cartTotal=Math.max(0,cartRawTotal-Number(promo.discount||0))+Number(promo.packagingFee||0)
  ...
  const shared={auth,catalog,addressBook,deliveryZones,destination,setDestination,selectedAddress,branch,storeStatus,promo}
...
function Header({auth,catalog,addressBook,deliveryZones,destination,setDestination,selectedAddress,plain=false,hideBrand=false}){
...
function HomePage({auth,catalog,addressBook,destination,setDestination,selectedAddress,add,cartCount,storeStatus,promo}){const nav=useNavigate();const featured=catalog.products.filter(p=>p.featured&&p.available!==false).slice(0,6);const branch=catalog.branches.find(b=>b.id===(selectedAddress?.branch_id||destination.branchId))||catalog.branches[0];return <main><Header auth={auth} catalog={catalog} addressBook={addressBook} destination={destination} setDestination={setDestination} selectedAddress={selectedAddress}/>{storeStatus?.ready&&!storeStatus?.open&&<StoreClosedBanner hours={catalog.settings?.business_hours}/>}<section className="hero"><div className="hero-copy"><span className="hero-location-tag">ZÁKIA · MILENIO</span><span className="eyebrow">KYO A TU MANERA</span><h1>Tu sushi favorito,<br/><em>más cerca de ti.</em></h1><p>Pide directo, acumula KYO Points y recibe beneficios exclusivos.</p><button className="primary" onClick={()=>nav('/menu')}>Ordenar ahora <ChevronRight size={18}/></button></div><div className="hero-art"><div className="red-orb"></div><img src="/assets/menu/ebi-crispy-ramen.jpg" alt="Ramen KYO"/></div></section><section className="quick-row"><div><Bike/><span><strong>Delivery</strong><small className="quick-time">{branch?.eta||'35–50 min'}</small></span></div><div><Store/><span><strong>Pickup</strong><small className="quick-time">20–30 min</small></span></div><div><Gift/><span><strong>Rewards</strong><small className="quick-time">1 punto por cada $10</small></span></div></section>{promo?.active&&<section className="promo-3x2-home active"><div><span className="promo-3x2-kicker">PROMO 3×2</span><h2>Combina rollos, ramen y pokes participantes</h2><p>Hoy está activa: agrega 3 participantes y el de menor precio va por cuenta de KYO.</p></div><button onClick={()=>nav('/menu?category=3x2')}>Aprovechar 3×2 <ChevronRight size={17}/></button></section>}<section className="section"><div className="section-head"><div><span className="eyebrow dark">LOS MÁS PEDIDOS</span><h2>Favoritos de KYO</h2></div><button className="text-btn" onClick={()=>nav('/menu')}>Ver todo <ChevronRight size={17}/></button></div><div className="product-scroller">{featured.map(p=><ProductCard key={p.id} p={p} add={add} branchId={branch?.id} storeOpen={storeStatus?.open} storeReady={storeStatus?.ready} promoActive={promo?.active}/>)}</div></section><section className="reward-banner"><div><span className="reward-icon"><Sparkles/></span><span><small>KYO REWARDS</small><strong>Come rico. Gana puntos.<br/>Recibe más KYO.</strong></span></div><button onClick={()=>nav('/rewards')}>Ver mis beneficios</button></section><footer className="home-legal-footer home-legal-footer-rich">
  ...
  <div className="home-footer-links">
    ...
    <button type="button" onClick={()=>nav('/privacy')}>Política de Privacidad</button>
    <span className="home-legal-dot">·</span>
    <button type="button" onClick={()=>nav('/terms')}>Términos y Condiciones</button>
  </div>
</footer>{cartCount>0&&<button className="floating-cart" onClick={()=>nav('/cart')}><ShoppingBag size={20}/><span>Ver carrito</span><b>{cartCount}</b></button>}</main>}
...
function ProductCard({p,add,branchId,storeOpen=true,storeReady=true,promoActive=false}){const [previewOpen,setPreviewOpen]=useState(false);const branchUnavailable=branchId&&p.branchAvailability?.[branchId]===false;const storeClosed=storeReady&&!storeOpen;const scheduleLoading=!storeReady;return <><article className={`product-card ${branchUnavailable||storeClosed?'branch-unavailable':''} ${p.promo_3x2_eligible&&promoActive?'promo-eligible':''}`}><div className="product-img product-img-previewable" onClick={()=>setPreviewOpen(true)} role="button" tabIndex={0} aria-label={`Ver imagen de ${p.name}`} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setPreviewOpen(true)}}}><LazyProductImage src={p.image||p.image_url} alt={p.name}/><span className="product-zoom-hint">Ver foto</span>{p.spicy&&<span className="spicy"><Flame size={13}/> Spicy</span>}{p.promo_3x2_eligible&&promoActive&&<span className="promo-product-badge active">3×2 HOY</span>}{(branchUnavailable||storeClosed)&&<span className="branch-soldout">{storeClosed?'Cerrado ahora':'No disponible aquí'}</span>}</div><div className="product-body"><small>{p.category}</small><h3>{p.name}</h3><p>{p.desc||p.description}</p>{p.promo_3x2_eligible&&promoActive&&<div className="promo-product-note">Combínalo con otros participantes · el más barato gratis</div>}<div><strong>{productDisplayPrice(p)}</strong><button className="add-btn" disabled={branchUnavailable||storeClosed||scheduleLoading} onClick={()=>!branchUnavailable&&!storeClosed&&!scheduleLoading&&add(p)} aria-label={`Agregar ${p.name}`}><Plus/></button></div></div></article>{previewOpen&&<ProductImagePreview product={p} onClose={()=>setPreviewOpen(false)}/>}</>}
...
function MenuPage(props){
  ...
  const promoActive=!!props.promo?.active
  const categoryList=['Favoritos',...(promoActive&&catalog.products.some(p=>p.promo_3x2_eligible)?['3x2']:[]),...catalog.categories]
  useEffect(()=>{if(!promoActive&&cat==='3x2')setCat('Favoritos')},[promoActive,cat])
...
function AddressModal({auth,deliveryZones,onClose,onSaved,initial=null}){
  const ownDeliveryZones=useDeliveryZones()
  const zoneSource=deliveryZones||ownDeliveryZones
  ...
  const save=async e=>{
    ...
    const zone=(zoneSource?.zones||[]).find(z=>z.name===form.neighborhood)
    if(!zone){setError('Selecciona una colonia disponible de la lista.');return}
    ...
    const payload={
      ...
      address_line:formatAddress({...form,neighborhood:zone.name})
    }
  ...
  return <div className="modal-backdrop">
    <form className="address-editor" onSubmit={save}>
      ...
      <div className="address-form-grid">
        ...
        <label className="admin-field full-span"><span>Colonia</span><select required value={zones.some(z=>z.name===form.neighborhood)?form.neighborhood:''} onChange={e=>{setForm({...form,neighborhood:e.target.value});setError('')}} disabled={zoneSource?.loading}><option value="">{zoneSource?.loading?'Cargando colonias...':'Selecciona tu colonia'}</option>{zones.map(z=><option key={z.id} value={z.name}>{z.name}</option>)}</select><small className="address-zone-note">La sucursal y el costo de envío se asignan automáticamente según tu colonia.</small></label>
        <label className="admin-field full-span"><span>Código postal</span><input required inputMode="numeric" value={form.postal_code} onChange={e=>setForm({...form,postal_code:e.target.value})}/></label>
      ...
      <div className="modal-actions">
        ...
        <button className="primary" disabled={busy||zoneSource?.loading}>{busy?'Guardando...':'Guardar dirección'}</button>
      </div>
    </form>
  </div>
}
function ProfilePage({auth,addressBook,deliveryZones,catalog,destination,setDestination,selectedAddress}){
  const nav=useNavigate(); const [showAddresses,setShowAddresses]=useState(false); const [showBranches,setShowBranches]=useState(false); const [adding,setAdding]=useState(false); const [editing,setEditing]=useState(null)
...
function StripePaymentForm({order,fulfillmentType,chargeTotal,onPaid,onBack}){
  ...
  const pay=async()=>{
    ...
    setBusy(true);setError('')
    const returnUrl=`${window.location.origin}/success?payment=return&order=${encodeURIComponent(order.id)}&number=${encodeURIComponent(order.order_number)}&type=${encodeURIComponent(fulfillmentType)}`
    const result=await stripe.confirmPayment({elements,confirmParams:{return_url:returnUrl},redirect:'if_required'})
    if(result.error){setError(result.error.message||'No pudimos procesar la tarjeta. Revisa los datos e intenta nuevamente.');setBusy(false);return}
    if(['succeeded','processing'].includes(result.paymentIntent?.status)){
      onPaid(order)
      return
    }
    setError('Stripe todavía no confirmó el pago. Intenta nuevamente.');setBusy(false)
  }
...
function CheckoutPage({cart,total,rawTotal,promo,auth,catalog,addressBook,deliveryZones,destination,setDestination,selectedAddress,branch,setCart,storeStatus}){
  ...
  const effectiveTipPercent=tipPercent
  const productTotalAfterPromo=Math.max(0,Number(rawTotal||0)-Number(promo?.discount||0))
  const packagingFee=Number(promo?.packagingFee||0)
  const tipAmount=Math.round((productTotalAfterPromo*effectiveTipPercent/100)*100)/100
  const orderTotal=productTotalAfterPromo+packagingFee+deliveryFee
...
function SuccessPage({setCart}){
  ...
  if(verifying)return <main className="success success-v2"><section className="success-card stripe-verifying-card"><RefreshCw className="spin" size={42}/><span className="success-kicker">VERIFICANDO PAGO</span><h1>Confirmando con Stripe…</h1><p className="success-lead">No cierres esta ventana. En cuanto Stripe confirme el cobro, enviaremos tu pedido a Cocina.</p></section></main>
  if(paymentError)return <main className="success success-v2"><section className="success-card"><div className="success-badge payment-warning"><CreditCard/></div><span className="success-kicker">ESTADO DEL PAGO</span><h1>Revisa tu pago</h1><p className="success-lead">{paymentError}</p><button className="success-home-btn" onClick={()=>nav('/orders')}>Ver mis pedidos <ChevronRight size={19}/></button><p className="success-orders-hint"><button onClick={()=>nav('/checkout')}>Volver al checkout</button></p></section></main>
  return <main className="success success-v2">
    <div className="success-glow success-glow-one"></div><div className="success-glow success-glow-two"></div><section className="success-card"><div className="success-badge"><Check/></div><span className="success-kicker">PEDIDO CONFIRMADO</span><h1>¡Ya lo estamos preparando!</h1><p className="success-lead">{pickup?'Cocina ya recibió tu pedido. Te avisaremos cuando esté listo para que pases por él.':'Gracias por pedir directo con KYO. Cocina ya recibió tu orden y empezó a trabajar en ella.'}</p>{orderNumber&&<div className="success-order-number"><small>NÚMERO DE PEDIDO</small><strong>#{orderNumber}</strong></div>}<div className="success-status-card"><div className="success-status-head"><span className="success-live-dot"></span><div><small>ESTADO ACTUAL</small><strong>Preparando</strong></div></div><div className="success-track"><div className="success-track-step active"><span><Check size={15}/></span><div><strong>Preparando</strong><small>Cocina tiene tu pedido</small></div></div><div className="success-track-line"></div><div className="success-track-step"><span>2</span><div><strong>{pickup?'Listo para recoger':'En camino'}</strong><small>{pickup?'Te avisaremos cuando esté listo':'Te avisaremos cuando salga'}</small></div></div><div className="success-track-line"></div><div className="success-track-step"><span>3</span><div><strong>Entregado</strong><small>¡A disfrutar KYO!</small></div></div></div></div><div className="success-note"><Clock3/><span><strong>Todo listo.</strong><small>{pickup?'Espera a que tu pedido cambie a “Listo para recoger”.':'No necesitas hacer nada más por ahora.'}</small></span></div><button className="success-home-btn" onClick={()=>nav('/')}>Volver a inicio <ChevronRight size={19}/></button><p className="success-orders-hint">En <button onClick={()=>nav('/orders')}>Pedidos</button> puedes rastrear tu pedido.</p></section>
  </main>
}
