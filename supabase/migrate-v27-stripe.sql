-- KYO v27 — Stripe (tarjeta + Apple Pay + webhook)
-- Ejecutar DESPUÉS de migrate-v26-envio-gratis-propina.sql.
--
-- Flujo:
-- efectivo -> pedido entra directo a preparing
-- tarjeta  -> pedido nace pending_payment y SOLO el webhook de Stripe lo mueve a preparing
--
-- La propina permanece separada:
-- total = venta KYO
-- tip_amount = propina repartidor
-- payment_total = monto real cobrado por Stripe

-- Permitir el estado temporal de pago sin exponer pedidos sin cobrar a Cocina.
alter table public.orders drop constraint if exists orders_status_check;
alter table public.orders add constraint orders_status_check check (status in ('received','accepted','pending_payment','preparing','ready','on_the_way','delivered','cancelled'));
...
alter table public.orders add column if not exists save_payment_method boolean not null default false;

alter table public.orders drop constraint if exists orders_payment_status_check;
alter table public.orders add constraint orders_payment_status_check check (payment_status in ('not_required','pending','paid','failed'));
create unique index if not exists orders_stripe_payment_intent_uidx on public.orders(stripe_payment_intent_id) where stripe_payment_intent_id is not null;

-- IDs de Customer se guardan por sucursal/cuenta Stripe. No contienen datos de tarjeta.
alter table public.profiles add column if not exists stripe_customer_ids jsonb not null default '{}'::jsonb;

-- Pedidos históricos de efectivo/no Stripe no requieren pago online.
update public.orders set payment_status='not_required' where payment_method is distinct from 'card' and payment_status is distinct from 'not_required';

-- La función del cliente nunca puede marcar por sí sola un pedido con tarjeta como pagado.
...
begin
  ...
  if p_payment_method not in ('cash','card') then raise exception 'Método de pago inválido'; end if;
  ...
  -- v26: envío siempre gratis. `total` representa exclusivamente la venta de KYO,
  -- sin propina, para que reportes, ticket promedio y KPIs no la contabilicen como ingreso.
  v_delivery_fee:=0;
  v_total:=v_subtotal;
  ...
  insert into public.orders(
    ...
    delivery_notes,payment_method,status,subtotal,delivery_fee,total,tip_percentage,tip_amount,payment_total,payment_status
  )
  values(
    ...
    p_delivery_notes,p_payment_method,case when p_payment_method='card' then 'pending_payment' else 'preparing' end,v_subtotal,v_delivery_fee,v_total,v_tip_percentage,v_tip_amount,v_payment_total,case when p_payment_method='card' then 'pending' else 'not_required' end
  )
